...
namespace Lucky.Utilities
{
    public static class MathUtils
    {
        ...
        /// 直角三角形的斜边
        /// </summary>
        public static float Hypotenuse(float x, float y) => Pow(Pow(x, 2) + Pow(y, 2), 0.5f);
        ...
        public static float SnapAngleToNearestAxis(float degree)
        {
            float remain = Mod(degree, 90);
            if (remain < 45)
                return degree - remain;
            return degree + 90 - remain;
        }

        public static float SnapAngleToGivenAxis(float degree, Vector3 to, Vector3 baseAxis, Vector3 rotateAxis)
        {
            Vector3 curretnVector = Quaternion.AngleAxis(degree, rotateAxis) * baseAxis;
            float deltaAngle = Vector3.SignedAngle(curretnVector, to, rotateAxis);
            return degree + deltaAngle;
        }

        public static Vector3 RotateY90(this Vector3 vector)
        {
