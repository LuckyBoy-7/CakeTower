...
{
    public class DefaultDict<TKey, TVal> : IDictionary<TKey, TVal>, ICloneable
    {
        ...
        public DefaultDict(Func<TVal> getter)
        {
            ...
            _getter = getter ?? throw new ArgumentNullException("DefaultDict needs a getter!");
        }
        ...
        public object Clone()
        {
            DefaultDict<TKey, TVal> other = new DefaultDict<TKey, TVal>(_getter);
            foreach ((TKey key, TVal value) in _dic)
            {
                other._dic[key] = value;
            }
